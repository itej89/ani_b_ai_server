        ...
        # Send the prediction back
            ...
            try:
                ...
                if question != "":
                    ...
                    print("---------------------------------------------------")
                    print(f"BotChat---question-------:{question}")
                    print("---------------------------------------------------")
                    answer = EndPointManager.BotChat(question, QHighestEmotion)
                    print(f"BotChat---answer-------:{answer}")


                    print('ChatBot Response : {}'.format(answer['DATA']['RESPONSE']))

                    # emotion = answer['DATA']['EMO_DATA']

                    emotion = EndPointManager.BotEmotion(answer['DATA']['RESPONSE'])['DATA']
                    print(f"BotChat---emotion-------:{emotion}")
                    ...
                    if nlp_information['CONFIDENCE'] > 0.71:
                        ...
                        audio_wav_bytes = EndPointManager.BotSynthesis(answer['DATA']['RESPONSE'])['DATA']
                        

                    Group(clientName).send({'text': json.dumps({'message': answer['DATA']['RESPONSE'], 'anger': emotion['ANGER'], 'disgust': emotion['DISGUST'], 'fear': emotion['FEAR'], 'joy': emotion['JOY'], 'sadness': emotion['SADNESS'], 'surprise': emotion['SURPRISE'], 'intent':nlp_information['INTENT'], 'intent_conf':nlp_information['CONFIDENCE'] ,'synth': audio_wav_bytes})})
